# Production status — MLB book spreads

Tracks which spreads are signed off as **done** (design locked, data verified,
photos placed at ≥300 DPI, vector PDF path wired). The user confirms completion
per spread; do not mark a spread done without their say-so.

Legend: ✅ done · ⬜ not yet confirmed

> **2026-06-27 — all sign-offs reset.** The interior export was re-sized to the
> correct Blurb Large Landscape Hardcover ImageWrap interior spec
> (page 12.625 × 10.875 in / 909 × 783 pt; trim 12.5 × 10.625 in; no binding-edge
> bleed; content now prints at true native scale instead of the old ~3% stretch).
> Every spread is back to ⬜ pending a full visual re-inspection. Prior
> confirmation notes are retained below for reference.

## Completed
- ✅ **Globe Life Field** — confirmed 2026-06-28 at new trim. Full glyph-bounds
  safe-area sweep clean (0 violations); fixed right-page roof-band running heads
  (band 52→78px, bottom-anchored) and left-page hero title overflow (`bottom`
  60→78px, tight `line-height: .86`). Plate 2 swapped to group-celebration photo,
  user crop persisted. Stadium-context prose collision resolved (leading
  1.54→1.38). Data fidelity + vector PDF path intact.

## Pending
### Ballpark spreads
- ⬜ American Family Field
- ⬜ Anaheim Stadium — _prior sign-off 2026-06-26: all five photos ≥300 DPI
  (hero ≈336 DPI; panels 556–855 DPI); p3 crop re-fit. Re-inspect at new trim._
- ⬜ Busch Stadium
- ⬜ Chase Field
- ⬜ Citi Field
- ⬜ Citizens Bank Park
- ⬜ Cleveland Municipal Stadium
- ⬜ Comerica Park
- ⬜ Comiskey Park
- ⬜ Coors Field
- ⬜ Fenway Park — _prior sign-off 2026-06-26: hero 300 DPI (4742×3264); four
  editorial photos ≥326 DPI; field instrument + folio + scrim final. Re-inspect
  at new trim._
- ✅ Globe Life Field — _see Completed above (confirmed 2026-06-28 at new trim)._
- ⬜ Great American Ball Park
- ⬜ Marlins Park
- ⬜ Memorial Stadium
- ⬜ Mile High Stadium
- ⬜ Milwaukee County Stadium
- ⬜ Minute Maid Park
- ⬜ Nationals Park
- ⬜ New Comiskey Park
- ⬜ Oracle Park
- ⬜ Oriole Park at Camden Yards
- ⬜ Petco Park
- ⬜ PNC Park — _prior sign-off 2026-06-26: five live image-slots placed (461–611
  DPI); wide hero 5000×3333 (≈306 DPI); stacked masthead + gold L-bracket title.
  Re-inspect at new trim._
- ⬜ Progressive Field
- ⬜ Riverfront Stadium
- ⬜ Rogers Centre
- ⬜ Royals Stadium
- ⬜ Safeco Field
- ⬜ San Diego Stadium
- ⬜ Shea Stadium
- ⬜ SunTrust Park
- ⬜ Sutter Health Park
- ⬜ Target Field
- ⬜ Three Rivers Stadium
- ⬜ Tiger Stadium
- ⬜ Tropicana Field
- ⬜ Veterans Stadium
- ⬜ Wrigley Field
- ⬜ Yankee Stadium
- ⬜ Yankee Stadium 2009

### Front/back matter & interstitials
- ⬜ Opening Spread
- ✅ **Closing Spread** — confirmed 2026-06-27. Single interior page (cover-grid,
  14 parks). All 14 photos ≥300 DPI: Citi (~343), Safeco (~325), Yankee 2009
  (~395) re-shot; Coors replaced with 1280×953 (441 DPI @ 1×, clean crop slate).
- ⬜ Road Trips Overview Spread
- ⬜ MLB Stadium Timeline Spread
