/* wrigley-protractor.jsx — Wrigley Field "Field Plan" graphic, protractor study.
   A Cubs-palette copy of the shared protractor diagram (ivy-green grass, brick
   warning track, restrained Cubs-blue bearing needle) so the other spreads stay
   untouched. A protractor scale reads the field-axis bearing off true north; the
   simple ballpark is rotated to match. Props: lf, cf, rf, orientation, degrees,
   accent. Exposes window.WrigleyProtractor. Local data only. */
(function () {
  const e = React.createElement;

  // Wrigley palette (warm charcoal + Cubs blue + ivy green + brick)
  const ink = "#1C1B19", black = "#19181A", blue = "#14346F", blueDeep = "#0E2A5C", blueBright = "#2A5BB0";
  const grass = "#4E6B3C", grassHi = "#5E7E49", clay = "#B98F54", clayEdge = "#9C6E3F";
  const brick = "#8C4A36", wallInk = "#5E3526";
  const paperHi = "#F1ECDF", rule = "#CDC6B5", ruleStrong = "#ADA590", ink3 = "#847E70", ink2 = "#555046";
  ...
  function Chip(props) {
    ...
    const fill = tone === "blue" ? blue : paperHi, stk = tone === "blue" ? blueDeep : rule, col = tone === "blue" ? "#F1ECDF" : ink;
    return e("g", null,
  ...
  function WrigleyProtractor(props) {
    const accent = props.accent || blue;
    const deg = (props.degrees != null) ? props.degrees : 37;
    ...
    // brick outfield wall arc (Wrigley signature) — just inside the grass edge
    const wlf = out(lf, C, -3), wrf = out(rf, C, -3);
    const wallPath = "M " + wlf[0] + " " + wlf[1] + " A " + (R - 3) + " " + (R - 3) + " 0 0 1 " + wrf[0] + " " + wrf[1];
    ...
    return e("div", { className: "fd-wrap" },
      e("svg", { viewBox: "62 28 300 290", className: "fd-svg", role: "img", "aria-label": "Field dimensions plan, oriented " + orientation + " " + deg + " degrees" },
        e("defs", null,
          e("linearGradient", { id: "wrProtGrass", x1: "0", y1: "0", x2: "0", y2: "1" },
            e("stop", { offset: "0", stopColor: grassHi }), e("stop", { offset: "1", stopColor: grass }))
        ),
        // field group (rotated)
        e("g", { transform: "rotate(" + deg + " " + C[0] + " " + C[1] + ")" },
          e("path", { d: grassPath, fill: "url(#wrProtGrass)", stroke: wallInk, strokeWidth: 1.6, strokeLinejoin: "round" }),
          // brick outfield wall (Wrigley's ivy wall)
          e("path", { d: wallPath, fill: "none", stroke: brick, strokeWidth: 3, strokeLinecap: "round", opacity: 0.95 }),
        ...
        ticks, nums,
        e("text", { x: nlab[0], y: nlab[1] - 9, textAnchor: "middle", style: { fontFamily: "'Oswald',sans-serif", fontWeight: 700, fontSize: "13px", fill: ink, letterSpacing: ".04em" } }, "N"),
  ...
  window.WrigleyProtractor = WrigleyProtractor;
})();
