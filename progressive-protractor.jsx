/* progressive-protractor.jsx — Progressive Field "Field Plan" graphic.
   A protractor scale reads the field-axis bearing off true north (N / 0°); the
   ballpark is rotated to match (here 0°, so true north is straight up the CF
   axis). Steel-drafting palette to match the Gateway Frame spread. Distances
   and orientation come straight from window.PROG.field — local data only.
   Props: lf, cf, rf, orientation, bearing, accent. Exposes window.ProgProtractor. */
(function () {
  const e = React.createElement;

  // Progressive palette (charcoal + painted steel + field green; navy/red accents)
  const ink = "#23262B", black = "#16181C", navy = "#0C2340", red = "#C8102E";
  const grass = "#54703F", grassHi = "#65814D", clay = "#B98A52", clayEdge = "#A2743F", wallInk = "#3C424B";
  const paperHi = "#F2F0EA", rule = "#C9C6BB", ruleStrong = "#ABA89C", ink3 = "#878D96", ink2 = "#4C525B";
  ...
  function Chip(props) {
    const fs = props.size || 15, w = Math.max(props.minW || 0, props.text.length * fs * 0.62 + (props.padX || 9) * 2), h = props.h || Math.round(fs * 1.5);
    const tone = props.tone || "paper";
    const fill = tone === "accent" ? navy : paperHi, stk = tone === "accent" ? navy : ruleStrong, col = tone === "accent" ? paperHi : ink;
    return e("g", null,
  ...
  function ProgProtractor(props) {
    const accent = props.accent || navy;
    const deg = (props.bearing != null) ? props.bearing : 0;
    ...
    // protractor scale, centered on the 0deg axis so a symmetric park reads square
    const ticks = [];
    ...
    const nums = [-60, -30, 0, 30, 60].map((a, i) => {
      const p = polar(C, PR + 14, a);
      return e("text", { key: "n" + i, x: p[0], y: p[1] + 5, textAnchor: "middle", style: { fontFamily: "'Space Mono',monospace", fontWeight: 700, fontSize: "14px", fill: ink3 } }, Math.abs(a));
    });
    const arcA = polar(C, PR, -60), arcB = polar(C, PR, 60);
    ...
    return e("div", { className: "fd-wrap" },
      e("svg", { viewBox: "20 28 320 290", className: "fd-svg", role: "img", "aria-label": "Field dimensions plan, oriented " + orientation + " " + deg + " degrees" },
        e("defs", null,
          e("linearGradient", { id: "progProtGrass", x1: "0", y1: "0", x2: "0", y2: "1" },
            e("stop", { offset: "0", stopColor: grassHi }), e("stop", { offset: "1", stopColor: grass }))
        ),
        e("g", { transform: "rotate(" + deg + " " + C[0] + " " + C[1] + ")" },
          e("path", { d: grassPath, fill: "url(#progProtGrass)", stroke: wallInk, strokeWidth: 1.6, strokeLinejoin: "round" }),
        ...
        ticks, nums,
        e("text", { x: nlab[0], y: nlab[1] - 10, textAnchor: "middle", style: { fontFamily: "'Oswald',sans-serif", fontWeight: 700, fontSize: "15px", fill: ink, letterSpacing: ".04em" } }, "N"),
  ...
  window.ProgProtractor = ProgProtractor;
})();
