/* american-protractor.jsx — American Family Field "field orientation" symbol.
   A compact architectural protractor that sits at the pivot hub of the roof
   fan: a half-circle scale reads the field-axis bearing off true north, the
   little ballpark is rotated to match (SE, 129deg). Brewers palette — navy
   needle, gold/brick chips, field green. Props: lf, cf, rf, orientation,
   degrees. Exposes window.AmFamProtractor. Local data only. */
(function () {
  const e = React.createElement;

  const ink = "#23211C", navy = "#13294B", navyDeep = "#0E1E38", brick = "#9E3B2E",
        gold = "#B68A2E", grass = "#5E7E54", grassHi = "#6E8B5E", clay = "#C0925A",
        clayEdge = "#A6793F", wallInk = "#384A33", paperHi = "#F4EDDD",
        rule = "#C8BDA6", ruleStrong = "#A99C80", ink3 = "#8C8470", ink2 = "#564E40";
  ...
  function Chip(props) {
    const fs = props.size || 13, w = Math.max(props.minW || 0, props.text.length * fs * 0.64 + (props.padX || 8) * 2), h = Math.max(20, fs + 8);
  ...
  function AmFamProtractor(props) {
    const deg = (props.degrees != null) ? props.degrees : 129;
    const orientation = props.orientation || "SE";
    const C = [180, 188], R = 86, PR = 122;
    ...
    // protractor scale spanning 0..150 so the SE (129) bearing is enclosed
    const ticks = [];
    for (let a = 0; a <= 150; a += 10) {
      const big = a % 30 === 0;
    ...
    return e("div", { className: "fd-wrap" },
      e("svg", { viewBox: "26 36 308 286", className: "fd-svg", role: "img", "aria-label": "Field dimensions plan, oriented " + orientation + " " + deg + " degrees" },
        e("defs", null,
          e("linearGradient", { id: "amfProtGrass", x1: "0", y1: "0", x2: "0", y2: "1" },
            e("stop", { offset: "0", stopColor: grassHi }), e("stop", { offset: "1", stopColor: grass }))
        ),
        e("g", { transform: "rotate(" + deg + " " + C[0] + " " + C[1] + ")" },
          e("path", { d: grassPath, fill: "url(#amfProtGrass)", stroke: wallInk, strokeWidth: 1.5, strokeLinejoin: "round" }),
        ...
        e(Chip, { x: bc.x, y: bc.y, text: deg + "\u00b0 " + orientation, size: 17, tone: "accent", padX: 7 }),
        chips.map((c, i) => e(Chip, { key: "d" + i, x: c.p[0], y: c.p[1], text: c.t, size: 17 }))
      )
    );
  }

  window.AmFamProtractor = AmFamProtractor;
})();
