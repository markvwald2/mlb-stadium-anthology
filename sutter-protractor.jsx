/* sutter-protractor.jsx — Sutter Health Park field-geometry graphic, rendered as
   a small engineering survey instrument (matches the house protractor family).
   A protractor scale reads the field-axis bearing off true north; the field is
   rotated to match (NE / 46deg). Charcoal structure, restrained A's-green grass,
   controlled Tower-Bridge-yellow bearing needle.
   Props: lf, cf, rf, orientation, degrees. Exposes window.SutterProtractor. */
(function () {
  const e = React.createElement;

  const ink = "#1E1C18", ink2 = "#4F4A42", ink3 = "#8A8273";
  const yellow = "#D79A22", yellowDeep = "#B07C16", black = "#17150F";
  const grass = "#4C6B4E", grassHi = "#5A7A5A", clay = "#B8895C", clayEdge = "#9C6F45", wallInk = "#2E3F2E";
  const paperHi = "#F1ECDD", rule = "#CBC3B0", ruleStrong = "#A99F88";
  ...
  function Chip(props) {
    ...
    const fill = tone === "yellow" ? yellow : paperHi, stk = tone === "yellow" ? yellowDeep : rule, col = tone === "yellow" ? "#2A2007" : ink;
    return e("g", null,
  ...
  function SutterProtractor(props) {
    const accent = yellow;
    ...
    return e("div", { className: "fd-wrap" },
      e("svg", { viewBox: "79 30 294 290", className: "fd-svg", role: "img", "aria-label": "Field dimensions plan, oriented " + orientation + " " + deg + " degrees" },
        ...
        e("g", { transform: "rotate(" + deg + " " + C[0] + " " + C[1] + ")" },
          e("path", { d: grassPath, fill: "url(#sutterProtGrass)", stroke: wallInk, strokeWidth: 1.6, strokeLinejoin: "round" }),
        ...
        e(Chip, { x: degPos[0], y: degPos[1], text: orientation + " " + deg + "\u00b0", size: 14, tone: "yellow", padX: 7 }),
        chips.map((c, i) => e(Chip, { key: "d" + i, x: c.p[0], y: c.p[1], text: c.t, size: 15 }))
      )
    );
  }

  window.SutterProtractor = SutterProtractor;
})();
