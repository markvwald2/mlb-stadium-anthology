...
(function () {
  ...
  function Chip(props) {
    const fs = props.size || 15, w = Math.max(props.minW || 0, props.text.length * fs * 0.62 + (props.padX || 9) * 2), h = props.h || Math.round(fs + 8);
  ...
  function GlobeProtractor(props) {
    ...
    const nums = [0, 60, 90].map((a, i) => {
      ...
      return e("text", { key: "n" + i, x: p[0], y: p[1] + 4 + (a === 0 ? 15 : 0), textAnchor: "middle", style: { fontFamily: "'Space Mono',monospace", fontWeight: 700, fontSize: "26px", fill: ink3 } }, a);
    });
    const arcA = polar(C, PR, 0), arcB = polar(C, PR, 110);
    const protArc = "M " + arcA[0] + " " + arcA[1] + " A " + PR + " " + PR + " 0 0 1 " + arcB[0].toFixed(1) + " " + arcB[1].toFixed(1);
    const ntip = polar(C, PR + 32, deg);
