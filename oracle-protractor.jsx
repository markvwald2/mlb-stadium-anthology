/* oracle-protractor.jsx — Oracle Park "Field Geometry" graphic, house protractor
   style (matches PNC / Royals / Coors). A protractor scale reads the field-axis
   bearing off true north; the ballpark is rotated to match (E / 85deg). Warm
   charcoal structure, bay-influenced sage grass, restrained Giants-orange needle.
   Props: lf, cf, rf, orientation, degrees, accent. Exposes window.OracleProtractor. */
(function () {
  const e = React.createElement;

  // Oracle palette (warm charcoal + faded Giants orange + bay-sage field)
  const ink = "#1C1B19", ink2 = "#514C44", ink3 = "#837C6E";
  const orange = "#BC6233", orangeDeep = "#9C4E27", black = "#17171A";
  const grass = "#6F8C5C", grassHi = "#7E9A6A", clay = "#C0A06A", clayEdge = "#A98750", wallInk = "#3C4A32";
  const paperHi = "#F1ECDF", rule = "#CCC5B4", ruleStrong = "#ABA38D";
  ...
  function Chip(props) {
    const fs = props.size || 15, w = Math.max(props.minW || 0, props.text.length * fs * 0.62 + (props.padX || 9) * 2), h = fs + 10;
    const tone = props.tone || "paper";
    const fill = tone === "orange" ? orange : paperHi, stk = tone === "orange" ? orangeDeep : rule, col = tone === "orange" ? "#FBF4E6" : ink;
    return e("g", null,
  ...
  function OracleProtractor(props) {
    const accent = props.accent || orange;
    const deg = (props.degrees != null) ? props.degrees : 85;
    ...
    const bc = window.FieldLabels.bearingChip(C, PR + 16, deg, 54, 24);
    ...
    return e("div", { className: "fd-wrap" },
      e("svg", { viewBox: "79 30 294 290", className: "fd-svg", role: "img", "aria-label": "Field dimensions plan, oriented " + orientation + " " + deg + " degrees" },
        e("defs", null,
          e("linearGradient", { id: "oracleProtGrass", x1: "0", y1: "0", x2: "0", y2: "1" },
        ...
        e("g", { transform: "rotate(" + deg + " " + C[0] + " " + C[1] + ")" },
          e("path", { d: grassPath, fill: "url(#oracleProtGrass)", stroke: wallInk, strokeWidth: 1.6, strokeLinejoin: "round" }),
        ...
        // north baseline (0deg) + bearing needle
        ...
        e(Chip, { x: bc.x, y: bc.y, text: orientation + " " + deg + "\u00b0", size: 19, tone: "orange", padX: 7 }),
        // distance chips (rotate with the field)
        chips.map((c, i) => e(Chip, { key: "d" + i, x: c.p[0], y: c.p[1], text: c.t, size: 19 }))
      )
    );
  }

  window.OracleProtractor = OracleProtractor;
})();
