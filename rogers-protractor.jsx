/* rogers-protractor.jsx — Rogers Centre "Field Dimensions & Orientation" plate.
   A protractor study: a half-circle scale reads the field-axis bearing off true
   north (NNW / 345°); the simple ballpark fan is rotated to match. Royal/Navy
   blueprint palette on warm paper, restrained red bearing needle.
   Props: lf, cf, rf, orientation, bearing, accent. Exposes window.RogersProtractor. */
(function () {
  const e = React.createElement;

  // blueprint palette
  const ink = "#22262C", ink2 = "#535A62", ink3 = "#8A8E92";
  const royal = "#1453a0", navy = "#0e2a55", red = "#b4232a";
  const grass = "#5c7e57", grassHi = "#6b8d61", clay = "#c2965f", clayEdge = "#a87c45", wallInk = "#36432f";
  const paperHi = "#F4EEDF", rule = "#cdc6b3", ruleStrong = "#ada58c";

  // measured CW from true north (up): x = sin, y = -cos
  ...
  function Chip(props) {
    ...
    const fill = tone === "accent" ? red : paperHi, stk = tone === "accent" ? "#8d1b21" : "#b6ac95", col = tone === "accent" ? paperHi : ink;
    return e("g", null,
  ...
  function RogersProtractor(props) {
    const accent = props.accent || red;
    const bearing = (props.bearing != null) ? props.bearing : 345;
    const orientation = props.orientation || "NNW";
    // rotate the field so CF points along the bearing (e.g. 345° = -15°)
    const deg = bearing - 360;
    const C = [165, 182], R = 80, PR = 120;

    // field geometry (local: home bottom a=180, CF top a=0, LF a=-90, RF a=90)
    ...
    // protractor scale: 270° → 90° over the top (a from -90 to +90)
    const ticks = [];
    for (let a = -90; a <= 90; a += 5) {
      ...
      ticks.push(e("line", { key: "t" + a, x1: p1[0], y1: p1[1], x2: p2[0], y2: p2[1],
        stroke: big ? navy : ruleStrong, strokeWidth: big ? 1.3 : 0.8, opacity: big ? 0.92 : 0.5 }));
    }
    ...
    return e("div", { className: "fd-wrap" },
      e("svg", { viewBox: "8 30 314 248", className: "fd-svg", role: "img",
        ...
        e("defs", null,
          e("linearGradient", { id: "rcProtGrass", x1: "0", y1: "0", x2: "0", y2: "1" },
            e("stop", { offset: "0", stopColor: grassHi }), e("stop", { offset: "1", stopColor: grass }))
        ),
        // rotated field
        e("g", { transform: "rotate(" + deg + " " + C[0] + " " + C[1] + ")" },
          e("path", { d: grassPath, fill: "url(#rcProtGrass)", stroke: wallInk, strokeWidth: 1.5, strokeLinejoin: "round" }),
  ...
  window.RogersProtractor = RogersProtractor;
})();
