/* protractor-diagram.jsx — Royals Stadium "Field Plan" graphic (study #3).
   Self-contained: a protractor scale reads the field-axis bearing off true
   north; the simple ballpark is rotated to match. Same props as the old
   FieldDiagram (lf, cf, rf, orientation, degrees, accent). Local data only. */
(function () {
  const e = React.createElement;

  // palette (Royals system)
  const royal = "#134A8E", royalDeep = "#0C2F5C", goldHi = "#D2B477", goldDeep = "#9A7C44", gold = "#BD9B60";
  const grass = "#557F54", grassHi = "#638F60", clay = "#C2965E", clayEdge = "#A87C45", chalk = "#F4EEE0", wallInk = "#33523B";
  const paperHi = "#F6F1E4", rule = "#C9BD9D", ruleStrong = "#B3A684", ink3 = "#8A93A0", slate = "#48586A";
  ...
  function Chip(props) {
    const fs = props.size || 15, w = Math.max(props.minW || 0, props.text.length * fs * 0.62 + (props.padX || 9) * 2), h = Math.max(22, fs + 6);
    const tone = props.tone || "paper";
    const fill = tone === "gold" ? gold : paperHi, stk = tone === "gold" ? goldDeep : rule, col = tone === "gold" ? royalDeep : royal;
    return e("g", null,
  ...
  function ProtractorDiagram(props) {
    const accent = props.accent || royal;
    // fontScale: enlarge label type in place (no geometry change) so small
    // figures still clear the print type-size floors. Default 1 (unchanged).
    const fscale = props.fontScale || 1;
    ...
    const mound = [C[0], C[1] + R * 0.42], d = R * 0.30;
    ...
    for (let a = 0; a <= 110; a += 10) {
      ...
      ticks.push(e("line", { key: "t" + a, x1: p1[0], y1: p1[1], x2: p2[0], y2: p2[1], stroke: big ? royal : ruleStrong, strokeWidth: big ? 1.4 : 0.9, opacity: big ? 0.9 : 0.55 }));
    }
    const nums = [0, 30, 60, 90].map((a, i) => {
      const p = polar(C, PR + 14, a);
      return e("text", { key: "n" + i, x: p[0], y: p[1] + 5, textAnchor: "middle", style: { fontFamily: "'Space Mono',monospace", fontWeight: 700, fontSize: (14 * fscale) + "px", fill: ink3 } }, a);
    });
    ...
    return e("div", { className: "fd-wrap" },
      e("svg", { viewBox: "62 28 280 290", className: "fd-svg", role: "img", "aria-label": "Field dimensions plan, oriented " + orientation + " " + deg + " degrees" },
        e("defs", null,
          e("linearGradient", { id: "protGrass", x1: "0", y1: "0", x2: "0", y2: "1" },
            e("stop", { offset: "0", stopColor: grassHi }), e("stop", { offset: "1", stopColor: grass }))
        ),
        // field group (rotated)
        e("g", { transform: "rotate(" + deg + " " + C[0] + " " + C[1] + ")" },
          e("path", { d: grassPath, fill: "url(#protGrass)", stroke: wallInk, strokeWidth: 1.6, strokeLinejoin: "round" }),
        ...
        ticks, nums,
        e("text", { x: nlab[0], y: nlab[1] - 10, textAnchor: "middle", style: { fontFamily: "'Oswald',sans-serif", fontWeight: 700, fontSize: (15 * fscale) + "px", fill: royal, letterSpacing: ".04em" } }, "N"),
        // north baseline (0°) + bearing needle
        e("line", { x1: C[0], y1: C[1], x2: polar(C, PR, 0)[0], y2: polar(C, PR, 0)[1], stroke: slate, strokeWidth: 1.1, strokeDasharray: "3 3" }),
        ...
        e(Chip, { x: bc.x, y: bc.y, text: deg + "\u00b0", size: 20 * fscale, tone: "gold", padX: 7 }),
        // distance chips
        chips.map((c, i) => e(Chip, { key: "d" + i, x: c.p[0], y: c.p[1], text: c.t, size: 20 * fscale }))
      )
    );
  }

  window.ProtractorDiagram = ProtractorDiagram;
})();
