/* camden-protractor.jsx — Oriole Park at Camden Yards "Field Plan" graphic,
   protractor study. A Camden-palette copy of the shared protractor diagram so
   the other stadiums stay untouched. A protractor scale reads the field-axis
   bearing off true north (NNE / 31°); the simple ballpark is rotated to match.
   Props: lf, cf, rf, orientation, bearing, accent. Exposes window.CamdenProtractor.
   Local data only — distances/orientation come straight from window.CAMDEN.field. */
(function () {
  const e = React.createElement;

  // Camden palette (warm charcoal + brick + weathered green steel + Orioles orange)
  const ink = "#211E18", black = "#16140F", orange = "#DF5A1E", orangeDeep = "#C24A14";
  const grass = "#4E6E48", grassHi = "#5C7E55", clay = "#BC9258", clayEdge = "#A87C45", wallInk = "#2D4435";
  const paperHi = "#F1EADB", rule = "#CEC5AF", ruleStrong = "#B0A691", ink3 = "#8C8270", ink2 = "#574F3F";
  ...
  function Chip(props) {
    const fs = props.size || 15, w = Math.max(props.minW || 0, props.text.length * fs * 0.62 + (props.padX || 9) * 2), h = Math.max(22, fs * 1.42);
    const tone = props.tone || "paper";
    const fill = tone === "accent" ? orange : paperHi, stk = tone === "accent" ? orangeDeep : rule, col = tone === "accent" ? paperHi : ink;
    return e("g", null,
  ...
  function CamdenProtractor(props) {
    const accent = props.accent || orange;
    const deg = (props.bearing != null) ? props.bearing : (props.degrees != null ? props.degrees : 31);
    ...
    const nums = [0, 60, 90].map((a, i) => {
      const p = polar(C, PR + 14, a);
      return e("text", { key: "n" + i, x: p[0], y: p[1] + 5, textAnchor: "middle", style: { fontFamily: "'Space Mono',monospace", fontWeight: 700, fontSize: "17px", fill: ink3 } }, a);
    });
    ...
    return e("div", { className: "fd-wrap" },
      e("svg", { viewBox: "20 28 320 290", className: "fd-svg", role: "img", "aria-label": "Field dimensions plan, oriented " + orientation + " " + deg + " degrees" },
        e("defs", null,
          e("linearGradient", { id: "cyProtGrass", x1: "0", y1: "0", x2: "0", y2: "1" },
            e("stop", { offset: "0", stopColor: grassHi }), e("stop", { offset: "1", stopColor: grass }))
        ),
        // field group (rotated)
        e("g", { transform: "rotate(" + deg + " " + C[0] + " " + C[1] + ")" },
          e("path", { d: grassPath, fill: "url(#cyProtGrass)", stroke: wallInk, strokeWidth: 1.6, strokeLinejoin: "round" }),
        ...
        ticks, nums,
        e("text", { x: nlab[0], y: nlab[1] - 12, textAnchor: "middle", style: { fontFamily: "'Oswald',sans-serif", fontWeight: 700, fontSize: "18px", fill: ink, letterSpacing: ".04em" } }, "N"),
        ...
        e(Chip, { x: bc.x + 19, y: bc.y, text: deg + "\u00b0", size: 24, tone: "accent", padX: 8 }),
        // distance chips
        chips.map((c, i) => e(Chip, { key: "d" + i, x: c.p[0], y: c.p[1], text: c.t, size: 25 }))
      )
    );
  }

  window.CamdenProtractor = CamdenProtractor;
})();
