...
(function () {
  ...
  const RETRO = [
    ...
    "Oracle Park and PNC Park refined the model. In San Francisco, Populous and the Giants exploited the waterfront site so thoroughly that the bay itself became part of the ballpark drama. The Giants\u2019 own history stresses mass transit, private financing, and the building\u2019s classic design. In Pittsburgh, Populous framed the Allegheny and skyline so well that the view turned into an argument for the whole typology. These parks were still nostalgic in material language, but they were modern in revenue logic: naming rights, premium seating, sponsorships, digital systems, and highly choreographed fan circulation. Retro-classic form, in practice, was compatible with increasingly sophisticated sports capitalism. The result was familiarity carefully engineered to feel spontaneous and authentic.",
  ];

  const DISTRICT = [
    "The newest MLB ballpark is often not a ballpark at all, at least not in the old sense. It is an anchor tenant in a larger development ecology. Its architecture must serve baseball, but also brand a neighborhood, support real-estate value, host non-game events, and justify financing packages built from taxes, fees, land deals, naming rights, premium inventory, and sponsor integration. The contemporary ballpark is both stage and platform, where entertainment, commerce, civic identity, and urban investment reinforce one another year-round.",
